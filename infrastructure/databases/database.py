...
DEUTSCH_DATABASE_URL = app_settings.database_url
if not DEUTSCH_DATABASE_URL:
    raise RuntimeError(
        "DEUTSCH_DATABASE_URL environment variable is required")

DEUTSCH_DATABASE_MIGRATION_URL = app_settings.database_migration_url
if not DEUTSCH_DATABASE_MIGRATION_URL:
    raise RuntimeError(
        "DEUTSCH_DATABASE_MIGRATION_URL environment variable is required")


parsed = urlparse(DEUTSCH_DATABASE_URL)
...
DEUTSCH_DATABASE_URL = urlunparse(parsed)

engine = create_async_engine(
    DEUTSCH_DATABASE_URL,
    pool_pre_ping=True,
    connect_args={
        "timeout": 30,
