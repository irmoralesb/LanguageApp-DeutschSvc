...
LanguageApp German Verbs service - FastAPI-based microservice using SQLAlchemy 2.x with async sessions and SQL Server 2022 as the backing store.

## Database drivers

### pyodbc (sync driver, used for migrations)
- Best compatibility with SQL Server features; leveraged by Alembic through `DEUTSCH_DATABASE_MIGRATION_URL`.
...
- Runtime uses SQLAlchemy async engine with the `DEUTSCH_DATABASE_URL` URL and automatically injects `LongAsMax=Yes` to preserve `MAX` semantics when pyodbc is the driver (see [database.py](database.py)).
...
- Alembic reads `DEUTSCH_DATABASE_MIGRATION_URL` for migrations, keeping a separate sync-safe URL for schema changes.
...
- `DEUTSCH_DATABASE_URL` -- async SQLAlchemy URL for application traffic (aioodbc).
- `DEUTSCH_DATABASE_MIGRATION_URL` -- sync SQLAlchemy URL for Alembic migrations (pyodbc).
