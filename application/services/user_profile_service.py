...
class UserProfileService:
    ...
    async def get_german_noun_selections(self, user_id: UUID) -> list[UserGermanNounSelectionModel]:
        ...
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.get_german_noun_selections(user_id)
    ...
    async def add_german_noun_selection(
        ...
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.add_german_noun_selection(user_id, german_noun_id)

    async def remove_german_noun_selection(self, user_id: UUID, german_noun_id: UUID) -> bool:
        existing = await self.profile_repo.get_by_user_id(user_id)
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.remove_german_noun_selection(user_id, german_noun_id)

    async def get_german_verb_selections(self, user_id: UUID) -> list[UserGermanVerbSelectionModel]:
        existing = await self.profile_repo.get_by_user_id(user_id)
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.get_german_verb_selections(user_id)
    ...
    async def add_german_verb_selection(
        ...
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.add_german_verb_selection(user_id, german_verb_id)

    async def remove_german_verb_selection(self, user_id: UUID, german_verb_id: UUID) -> bool:
        existing = await self.profile_repo.get_by_user_id(user_id)
        if existing is None:
            raise UserProfileNotFoundError(user_id)
        return await self.profile_repo.remove_german_verb_selection(user_id, german_verb_id)
