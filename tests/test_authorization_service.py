...
def user_for_service():
    svc = "deutsch-service-tests"
    return UserClaims(
        user_id=uuid4(),
        email="user@test.com",
        roles={svc: ["deutsch-user", "teacher"]},
    )
...
def test_check_role_returns_true_when_present(user_for_service):
    with patch(
        ...
        SimpleNamespace(service_name="deutsch-service-tests"),
    ):
        svc = AuthorizationService("deutsch-service-tests")
        assert svc.check_role(user_for_service, "deutsch-user") is True
...
def test_get_user_roles_explicit_service(user_for_service):
    svc = AuthorizationService("deutsch-service-tests")
    roles = svc.get_user_roles(user_for_service, service_name="deutsch-service-tests")
    assert roles == ["deutsch-user", "teacher"]
