# LanguageApp German Verbs Service

Backend API for **German verb practice** (conjugations by tense and person), **catalog of German verbs**, **user learning profiles**, **languages**, and **generated writing exercises**. Generation and grading use an **LLM** behind a **provider abstraction** (**LangChain** `init_chat_model`).

It is a consumer of the **Identity Service**: callers send **JWT access tokens** issued there. The **`service_name`** in settings must align with **`roles`** claim keys produced by Identity so **RBAC** works (e.g. **`german-verbs-service`** with roles like **`german-verbs-user`**).
...
flowchart TB
    ...
    subgraph germanVerbs["German Verbs Service"]
        subgraph api["application"]
            R["FastAPI routers\n(e.g. /api/v1/...)"]
            SCH["Pydantic schemas"]
        end
        subgraph app_svc["application/services"]
            PTC["GermanVerbCatalogService"]
    ...
    Web -->|"Bearer JWT"| germanVerbs
```

**JWT flow**

1. **`TokenService`** decodes Bearer tokens (**`secret_token_key`** and **`auth_algorithm`** must match Identity).
2. **`UserClaims`** holds **`roles`**: **`{ "<service-name>": ["role1", ...] }`** (for this API, typically **`german-verbs-service`**).
...
| **Validation** | Pydantic v2 models under **`application/schemas/`** (e.g. **`ExerciseGenerateRequest`**, **`ExerciseGenerateResponse`**). |
...
| **Persistence** | Async SQLAlchemy + **Azure SQL/SQL Server** via **`create_async_engine`** (**`DEUTSCH_DATABASE_URL`**). **`LongAsMax=Yes`** is appended automatically for ODBC compatibility. |
...
- **`SERVICE_ID`** and **`SERVICE_NAME`** registered consistently in Identity (service catalog / RBAC), e.g. **`german-verbs-service`**.
...
| Variable | Purpose |
| --- | --- |
| **`DEUTSCH_DATABASE_URL`** | Async **`mssql+aioodbc://...`** for the app |
| **`DEUTSCH_DATABASE_MIGRATION_URL`** | Sync **`mssql+pyodbc://...`** for Alembic |
...
1. Create the target database on SQL Server/Azure SQL (e.g. **`GermanVerbsDB`**).

2. Point **`DEUTSCH_DATABASE_MIGRATION_URL`** (Alembic) and **`DEUTSCH_DATABASE_URL`** (runtime) per **`.env_template`**.
...
Alembic uses **`DEUTSCH_DATABASE_MIGRATION_URL`** through **`alembic/env.py`**, models under **`infrastructure/databases/models.py`**, and **`Base.metadata`** for autogenerate.
...
Use **`--port`** that matches your **`LanguageApp-Web`** **`VITE_API_GERMAN_VERBS_URL`**.
...
| **`LanguageApp-Web`** | SPA with **German Verbs** practice UX (`VITE_API_GERMAN_VERBS_URL`) |
